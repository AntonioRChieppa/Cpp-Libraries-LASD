...
const Data & SetVec<Data>::Predecessor(const Data &value) const{
    if(size==0)
        throw std::length_error("Access to an empty SetVec!"); 
    unsigned long pos = 0;
    BinarySearch(pos,value); // O(log n)
    if(pos == 0)
        throw std::length_error("Predecessor not found!"); 
    else
        return Elements[CircularIndex(pos - 1)]; // O(1)
...
bool SetVec<Data>::Insert(const Data &value){
    if(size == capacity)
        Resize(capacity * 2); // O(1)

    unsigned long pos = 0;
    bool found = BinarySearch(pos,value); // O(log n)
    if(found){
    ...
    if (pos < size - pos) {
        ...
        for (unsigned long i = 0; i < pos; ++i)
            Elements[CircularIndex(i)] = std::move(Elements[CircularIndex(i + 1)]); // O(n)
